...
export default class Page31 extends Component {
	...
	loadDefaultData = async () => {
		...
		await axios.get("http://qa.mag.gob.sv/PRA/api/pantallas/get-informe-precios").then((res) => {
	...
	Add_and_UpdateData = async () => {
		...
		await axios
			.post("http://qa.mag.gob.sv/PRA/api/pantallas/add-subir-precios-internacional/" + Data.nombre + "/" + Data.Title + "/" + Data.Description + "", formData, config)
			.then((res) => {
				let API_Response = res.data;
				console.log(API_Response);
				if (API_Response === null || API_Response === undefined) {
					...
					this.SnackbarActions({ key: "Open", variant: "success", Message: "Saved", TimeOut: 1000 });
					this.loadDefaultData();
				} else {
					this.SnackbarActions({ key: "Open", variant: "warning", Message: "API Not responding.....", TimeOut: 1000 });
				}
	...
	UpdateStatusData = async (id, status) => {
		await this.setState({ BackDrop: true });
		await axios.put("http://qa.mag.gob.sv/PRA/api/pantallas/update-precio-informe/" + id + "/" + status).then((res) => {
			let API_Response = res.data;
			console.log(API_Response);
			if (API_Response === null || API_Response === undefined) {
				this.SnackbarActions({ key: "Open", variant: "warning", Message: "API Not responding.....", TimeOut: 1000 });
	...
	onChangeFiles = async (e) => {
		...
		Data.SelectedFileName = e.target.files.length > 0 ? e.target.files[0].name : null;
		await this.setState({ ModifiedData: Data });
	};
						...
						<Grid container spacing={2}>
							<Grid item xs={12}>
								<Grid container spacing={2}>
									...
									<Grid item xs={7}>
										<TextField variant="outlined" select fullWidth value={this.state.ModifiedData.nombre} onChange={(e) => this.TextFeildValueChanges(e.target.value, "nombre")}>
							...
							<Grid item xs={12}>
								<Grid container spacing={2}>
									...
									<Grid item xs={7}>
										<TextField variant="outlined" value={this.state.ModifiedData.Título} fullWidth onChange={(e) => this.TextFeildValueChanges(e.target.value, "Título")} />
									</Grid>
								</Grid>
							</Grid>
							<Grid item xs={12}>
								<Grid container spacing={2}>
									...
									<Grid item xs={7}>
										<TextField variant="outlined" multiline value={this.state.ModifiedData.Descripción} fullWidth onChange={(e) => this.TextFeildValueChanges(e.target.value, "Descripción")} />
									</Grid>
								</Grid>
							</Grid>
							<Grid item xs={12}>
								<Grid container spacing={2}>
									...
									<Grid item xs={7}>
										<TextField variant="outlined" fullWidth value={this.state.ModifiedData.SelectedFileName} type="file" onChange={(e) => this.onChangeFiles(e)} />
									</Grid>
								</Grid>
							</Grid>
						</Grid>

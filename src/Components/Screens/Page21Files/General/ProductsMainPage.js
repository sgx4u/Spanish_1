...
export default class ProductsMainPage extends Component {
	...
	loaddropdown3 = async (value) => {
		...
		await axios.get("https://siam-pra-1656956256760.azurewebsites.net/api/pracms/get-detalle-productos-categoria/" + value).then((res) => {
	...
	// Products dropdowns
	// 1=>https://siam-pra-1656956256760.azurewebsites.net/api/pracms/get-productores-lista
	// 2=>https://siam-pra-1656956256760.azurewebsites.net/api/pracms/categoris_lista
	// 3=>https://siam-pra-1656956256760.azurewebsites.net/api/pracms/get-detalle-productos-categoria/'string'(2nd API)
	// Save API Not there
	...
	DeleteData = async (idProductores) => {
		await this.setState({ BackDrop: true });
		await axios.post("https://siam-pra-1656956256760.azurewebsites.net/api/pracms/get-Detalle-productores-actualizar/" + idProductores).then((res) => {
			let API_Response = res.data;
			console.log(API_Response);
			if (API_Response === null || API_Response === undefined) {
				this.SnackbarActions({ key: "Open", variant: "warning", Message: "API Not responding.....", TimeOut: 1000 });
				...
				<Grid container spacing={2}>
							...
							<Grid item xs={12} sm={4}>
								...
								<TextField fullWidth select defaultValue={"Selecciona Productor"} value={this.state.dropdown1Value} onChange={(e) => this.LoadDatosProductores(e.target.value)}>									
									...
									{this.state.Dropdown1.map((data, Index) => {
										return (
											<MenuItem key={"1MIN" + Index} value={data.idProdctores}>
												{data.nombre}
											</MenuItem>
										);
									})}
							...
							<Grid item xs={12} sm={4}>
								<TextField fullWidth select defaultValue={"Selecciona Categoría de Productos"} value={this.state.dropdown2Value} onChange={(e) => this.loaddropdown3(e.target.value)}>
									...
									{this.state.Dropdown2.map((data, Index) => {
										return (
											<MenuItem key={"2MIN" + Index} value={data.nombre}>
												{data.nombre}
											</MenuItem>
										);
									})}
							...
							<Grid item xs={12} sm={4}>
								<TextField fullWidth select defaultValue={"Selecciona Producto"} value={this.state.dropdown3Value} onChange={(e) => this.setState({ dropdown3Value: e.target.value })}>
									...
									{this.state.Dropdown3.map((data, Index) => {
										return (
											<MenuItem key={"3MIN" + Index} value={data.idProducto}>
												{data.nombre}
											</MenuItem>
										);
									})}
					...
					<Grid item xs={12}>
						<Grid container spacing={2}>
							...
							<Grid item xs={6} sm={2}>
								<Button variant="outlined" color="secondary" style={{ marginTop: 10, float: "right" }} onClick={() => this.ClearData()}>
									Cancelar
								</Button>
							</Grid>
						</Grid>
										...
										return (
											<TableRow>
												...
												<TableCell align="center">
													<Button size="small" variant="outlined" color="primary" onClick={() => this.DeleteData(data.idProductores)}>
														Eliminar
													</Button>
												</TableCell>
											</TableRow>

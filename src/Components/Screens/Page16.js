...
export default class Page16 extends Component {
	...
	loadDefaultData = async () => {
		await this.setState({ BackDrop: true });
		await axios.get("http://qa.mag.gob.sv/PRA/api/pantallas/get-Usuario").then((res) => {
			console.log(res);
	...
	addData = async () => {
		...
		await axios.post("http://qa.mag.gob.sv/PRA/api/pantallas/add-usuario-activator/" + this.state.selectedData.idUsuario + "/" + (this.state.selectedData.checked === true ? 1 : 0)).then((res) => {
			console.log(res);
			let API_Response = res.data;
			console.log(API_Response);
			if (API_Response === null || API_Response === undefined) {
